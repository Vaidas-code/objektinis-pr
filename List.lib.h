...
void skait(studentas& tempas, int kiekis, list<studentas>& mas, string pav) {
	...
	try {
		...
		kiekis--;
		while (inputFile >> tempas.vardas >> tempas.pavarde) {
...
double mediana(list<int>& paz, studentas& tempas) {
	...
	list<int>::iterator it = paz.begin();
	advance(it, paz.size() / 2); // move iterator to middle element
	int n = paz.size();
	if (n % 2 == 0) {
		int elem1 = *prev(it); // get previous element
		int elem2 = *it;
		return (double)0.4 * ((elem1 + elem2) / 2) + 0.6 * tempas.egz;
	}
	else {
		return (double)0.4 * (*it) + 0.6 * tempas.egz;
	}
}
...
void sukurti1(studentas& tempas, list<studentas>& mas) {
	...
	list<studentas> abejingi;
	if (!egzistuojantis("sukurtas1000.txt") && !egzistuojantis("sukurtas10000.txt") && !egzistuojantis("sukurtas100000.txt") && !egzistuojantis("sukurtas1000000.txt") && !egzistuojantis("sukurtas10000000.txt"))
	{
